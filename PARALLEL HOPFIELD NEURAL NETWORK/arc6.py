...
for i in range(0, 2 ** a):
    ...
    z3 = np.sign((np.dot(Weights, z2)) - Threshold)
    diff = np.abs(z3 - V).all()
    if(diff == 0):
        print("\ni",i+1)
        print(z3)
        #print(v[i+1])
...
for i in range(0, 2 ** a):
    ...
    z11 = v[i+1]
    z12 = np.sign((np.dot(Weights1, z11)) - Threshold)
    z13 = np.sign((np.dot(Weights1, z12)) - Threshold)
    diff = np.abs(z13 - V).all()
    if(diff == 0):
        print("\ni",i+1)
        print(z13)
        #print(v[i+1])
